...
import com.fairy.templateapp.viewholders.HeaderViewHolder
import com.fairy.templateapp.viewholders.SimpleViewHolder
import com.fairy.templateapp.viewholders.SwitchViewHolder
...
class MainActivity : AppCompatActivity() {
    ...
    private fun onBindData(): (data: Any) -> RecyclerViewUIModel = { data ->
        when (data) {
            is String -> {
                HeaderViewHolder.HeaderUIModel(data)
            }
            is Animal -> {
                val icon = ContextCompat.getDrawable(this, data.icon)
                val title = getString(data.name)
                SimpleViewHolder.ListSimpleUIModel(icon, title, null)
            }
            is Vehicle -> {
                val icon = ContextCompat.getDrawable(this, data.icon)
                val title = getString(data.name)
                SimpleViewHolder.ListSimpleUIModel(icon, title, data.description)
            }
            is Device -> {
                val icon = ContextCompat.getDrawable(this, data.icon)
                val title = getString(data.name)
            ...
            is Option -> {
                SwitchViewHolder.SwitchUIModel(data.title, data.option, data.enabled) { isChecked ->
                    data.enabled = isChecked
                }
            }
            else -> {
                ViewHolderGenerator.EmptyUIModel()
            }
        }
    }
